...
| 3b | Gateway ネイティブ MCP サーバーの認証認可 詳細調査 | 完了 | [agentcore-gateway-mcp-auth.md](research/agentcore-gateway-mcp-auth.md) | AgentCore Gateway がネイティブ MCP サーバーをルーティングする際の認証認可フロー（Gateway ↔ MCP サーバー間の認証方式・認証コンテキストの伝播・対応スキーム一覧）を調査する。Task 6 着手前に完了することを推奨。Task 4・5 の進行中に並行して実施可 |
